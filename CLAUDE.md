...
| Screen never wakes on tilt; accelerometer reads exactly `x=0 y=0 z=0` | **The BMA423 silently reverted to power-on defaults** (`ACC_CONF` 0xA8→0x00, `PWR_CONF` 0x00→0x03, i.e. advanced power save back on) — seen 2026-08-25 after a USB unplug, so most likely a rail dip. An all-zero vector is physically impossible (gravity is always ~1 g in total), so `tilt_read()` treats 5 consecutive all-zero reads as "lost configuration" and re-runs init. Matters because screen wake compares the live vector against one captured at blank time: a stuck-at-zero accelerometer means the watch can never wake itself and the screen just stays black. Check with `bma` — it prints the config registers and a live reading whose magnitude must be ~1 g |
